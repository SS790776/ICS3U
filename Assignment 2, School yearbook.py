"""
    ...
    factorsarr = an array, with the returned value of function factors, holding half of 
        the factors of N
...
def factors(N):
    ...
    :return: returns an array of all of the factors of N up to the square root 
        (half of the factors)
...
while p == 1: #keep looping as long as p = 1
    ...
    try:
    #try to turn N into an integer, if python doesn't throw an exception, then continue
        N = int(N) 
        ...
        if N <= 0: #is x equal to or less than 0?
            ...
            """
            then call the function "factors" with paramiter N and assign it to factorsarr, 
            this will get us an array with half of the factors of N
            """
            ...
            #output the most efficient perimeter (2 multiplied by (best + ndbest)) 
                #and the dimensions "best" by "ndbest"
            print(f"The minimum perimiter is {2*(best + ndbest)}", end= " ")
            print(f"with dimensions {best}x{ndbest}. \n")
            p = 1 #and ask for user input again
    except ValueError: 
        #python threw an exception while trying to turn N into an integer, 
            #so N would not be an integer.

        #if the user inputted a variation of "done", we know they're trying to stop the program
